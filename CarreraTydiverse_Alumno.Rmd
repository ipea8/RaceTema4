...
```{r}
cran %>% filter(package == "plotly", r_version == "3.5.1") %>% select_all()
```
...
```{r}
cran3 <- cran %>% select(c(ip_id, package, size))
...
```{r}
cran3 %>% mutate(correct_size = 1000*size)
```
